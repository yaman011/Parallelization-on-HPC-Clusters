...
int main(int argc, const char *argv[]) {
  ...
  #pragma omp target data map(to: simdata) \
                          map(tofrom: simdata.pold[:NUMNODESTOT(simdata.pold->grid)], \
                                        simdata.pnew[:NUMNODESTOT(simdata.pnew->grid)], \
                                        simdata.vxold[:NUMNODESTOT(simdata.vxold->grid)], \
                                        simdata.vxnew[:NUMNODESTOT(simdata.vxnew->grid)], \
                                        simdata.vyold[:NUMNODESTOT(simdata.vyold->grid)], \
                                        simdata.vynew[:NUMNODESTOT(simdata.vynew->grid)], \
                                        simdata.vzold[:NUMNODESTOT(simdata.vzold->grid)], \
                                        simdata.vznew[:NUMNODESTOT(simdata.vznew->grid)])
  {
   
    #pragma omp target teams distribute parallel for
    for (int i = 0; i < 1; i++) {
        if (omp_get_thread_num() == 0 && omp_get_team_num() == 0) {
            printf("Number of GPU threads per team: %d\n", omp_get_num_threads());
            printf("Number of GPU teams: %d\n", omp_get_num_teams());
        }
    }
    for (int tstep = 0; tstep <= numtimesteps; tstep++) {
      ...
      if (simdata.params.outrate > 0 && (tstep % simdata.params.outrate) == 0) {
        #pragma omp target update from(simdata.pold[:NUMNODESTOT(simdata.pold->grid)])
...
void lattice_below(grid_t *grid, double x, double y, double z, int *cx, int *cy,
                   int *cz) {
  int m = (int)(floor(x));
  int n = (int)(floor(y));
  int p = (int)(floor(z));
...
int interpolate_inputmaps(simulation_data_t *simdata, grid_t *simgrid,
  ...
  for (int m = 0; m < simgrid->numnodesz; m++) {
    for (int n = 0; n < simgrid->numnodesy; n++) {
      for (int p = 0; p < simgrid->numnodesx; p++) {
